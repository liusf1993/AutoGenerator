...
//       return  c1.replaceAll("\\s","").equals(content.replaceAll("\\s",""));
  ...
  private void applySwitch() {
    SettingManager
        .applySwitch(CHK_managerUseBO.isSelected(), CHK_daoLogicDel.isSelected(), CHK_daoUseSeq.isSelected(), CHK_pageQuery.isSelected(),
            CHK_overwrite.isSelected(), CHK_generateBase.isSelected());
  }
  ...
  private void generate(String targetCode) {
    ...
    settingSave
        .setSwitch(CHK_managerUseBO.isSelected(), CHK_daoLogicDel.isSelected(), CHK_daoUseSeq.isSelected(), CHK_pageQuery.isSelected(),
            CHK_overwrite.isSelected(), CHK_generateBase.isSelected());
    ...
    switch (gt) {
      ...
      case ALL:
        ...
        r = generatorAdaptor.generate(GenerateType.Service);
        rs = getE(rs, r, GenerateType.Service.getName());
        r = generatorAdaptor.generate(GenerateType.ServiceImpl);
        rs = getE(rs, r, GenerateType.ServiceImpl.getName());
        r = generatorAdaptor.generate(GenerateType.Controller);
        rs = getE(rs, r, GenerateType.Controller.getName());
        break;
      default:
        break;
    }
    if (gt.equals(ALL)) {
      ...
      TEXT_msg.setText(
          "generate " + (TypePath.RESOURCES.equals(gt.getTypePath()) && !gt.equals(GenerateType.SQLMap) ? gt.getSuffix() + ".xml"
              : gt.getName()) + " " + (r ? "success" : "error" + ":" + FileWriter.getMsg()) + "!");
    }
  }
